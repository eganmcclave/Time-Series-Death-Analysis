...
nnetar_func <- function(train, test) {
  ...
  temp2 = temp^2
  trend = time(temp)
  fit <- nnetar(`Num Deaths`, p=2, xreg=cbind(trend, temp, temp2, `Ozone`,  `Relative Humidity`))
...
fcast$mean = ts(fcast$mean, start=c(2006, 356), frequency=365)
fcast$upper = ts(fcast$upper, start=c(2006, 356), frequency=365)
fcast$lower = ts(fcast$lower, start=c(2006, 356), frequency=365)

plot(fcast)
lines(test[,'Num Deaths'], col='red')

#nnetar_func(train, test)
```
