...
acfs_diff <- apply(diff(ts_vars), 2, acf, main='')

pacfs <- apply(ts_vars, 2, pacf, main='')
pacfs_diff <- apply(diff(ts_vars), 2, pacf, main='')
par(mfrow=c(1,1))
```
...
#df[which.max(ts_vars[,'Num Deaths']), 'Date']
-->
...
invisible(sapply(1:ncol(ts_vars), function(i_var) {
  monthplot(ts_vars[,i_var], ylab=colnames(ts_vars)[i_var])
}))

invisible(sapply(1:ncol(ts_vars), function(i_var) {
  monthplot(ts_vars[,i_var], ylab=colnames(ts_vars)[i_var], phase=cycles)
}))
...
invisible(sapply(colnames(ts_vars), function(var_name) {
  ...
  monthplot(ts_vars[,var_name], ylab='data', cex.axis=0.8, phase=cycles)
  monthplot(fit, choice='seasonal', cex.axis = 0.8, phase=cycles)
  monthplot(fit, choice = "trend", cex.axis = 0.8, phase=cycles)
  monthplot(fit, choice = "remainder", type = "h", cex.axis = 0.8, phase=cycles)
}))
par(mfrow=c(1,1))
```
