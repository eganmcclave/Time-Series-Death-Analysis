...
attach(as.data.frame(ts_vars))
temp = Temperature - mean(Temperature)
temp2 = temp^2
trend = time(temp)

fit1 = lm(`Num Deaths` ~ trend + temp + temp2 + Ozone + `Relative Humidity`, na.action=NULL)
plot(resid(fit1)); abline(h=0, col='red', lwd=2)
qqnorm(resid(fit1)); qqline(resid(fit1))
invisible(acf2(resid(fit1), 365))

fit2 = 
  arima(`Num Deaths`, order = c(4,0,14), xreg = cbind(trend, temp, temp2, Ozone, `Relative Humidity`))
...
fit3 = auto.arima(`Num Deaths`, xreg=cbind(trend, temp, temp2, Ozone, `Relative Humidity`),
...
#MTS::sVARMACpp()
fit4 <- MTS::sVARMACpp(ts_vars, c(1, 0, 1), c(3, 0, 9), s=365)
```
