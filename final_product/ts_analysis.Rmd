...
ts_vars <- ts(df[,c(2,6:7,4:5,8)], start=c(2002, 1), frequency=365.25)
...
bind <- cbind(trend, wday=train[,'Day of Week'])
...
###### Testing portion
trend = time(test)
bind <- cbind(trend, wday=test[,'Day of Week'])
...
rownames(tab) <- c(paste0(rep(colnames(train_var), 3), 
  ' lag ', rep(1:3, each=ncol(train_var))), 'trend', 'Day of Week')
...
#do.call(cbind, lapply(var_fit_1$varresult, coef))
```

```{r}
plot(fcast <- forecast(var_fit, h=nrow(test)))
...
# Plot ACF/PACFk
invisible(acf2(resid(fit), 365.25))
